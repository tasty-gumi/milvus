...
GeoH3Index::GeoH3Index(const storage::FileManagerContext& file_manager_context,
                       int resolution)
    : ScalarIndex<std::string>(H3_INDEX_TYPE),
      is_built_(false),
...
GeoH3Index::Serialize(const Config& config) {
    ...
    LOG_INFO("Start serilize H3 index data");
    ...
    LOG_INFO("The H3 serialized index data size:{}", index_data_size);
    res_set.Append(H3_INDEX_NULL_OFFSET, index_null_offsets, byte_size);
    LOG_INFO("The H3 serialized index null offsets data size:{}",
             index_data_size);
...
GeoH3Index::Load(milvus::tracer::TraceContext ctx, const Config& config) {
    ...
    LOG_INFO("The index file :{}", index_files.value());
...
// get the minimum hexagonal(as well as maximal resolution) that can contain all points of this geometry
// ,now only support point ,linstring ,polygon
void
...
GeoH3Index::Build(const Config& config) {
    LOG_INFO("start building H3 index");
    if (is_built_) {
        return;
    }
    auto insert_files =
...
GeoH3Index::In(size_t n, const std::string* values) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        //obtain every parent resolution,find the related parent H3Index
        for (auto iter = index_data_.begin(); iter != index_data_.end();
             ++iter) {
            H3Index parent;
            H3Error err = Success;
            if (getResolution(iter->first) > rep_res) {
                err = cellToParent(iter->first, rep_res, &parent);
                AssertInfo(err = Success,
                           "error when getting parent! the code {}",
                           err);
                if (parent == rep_index) {
                    for (auto& offset : *iter->second) {
                        res.set(offset);
            ...
            } else if (getResolution(iter->first) < rep_res) {
                err =
                    cellToParent(rep_res, getResolution(iter->first), &parent);
                AssertInfo(err = Success,
                           "error when getting parent! the code {}",
                           err);
...
GeoH3Index::Upload(const Config& config) {
    LOG_INFO("Start upload H3 index");
