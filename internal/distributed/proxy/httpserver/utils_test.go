...
func compareRow(m1 map[string]interface{}, m2 map[string]interface{}) bool {
	for key, value := range m1 {
		...
		} else if key == "field-json" || key == "field-geospatial" {
			arr1 := value.([]byte)
			arr2 := m2[key].([]byte)
			if len(arr1) != len(arr2) {
				return false
	...
	for key, value := range m2 {
		if (key == FieldBookIntro) || (key == "field-json") || (key == "field-geospatial") || (key == "field-array") {
			continue
		} else if strings.HasPrefix(key, "array-") {
			continue
		} else if value != m1[key] {
...
func newSearchResult(results []map[string]interface{}) []map[string]interface{} {
	for i, result := range results {
		...
		result["field-geospatial"] = []byte(`POINT (30.123 -10.456)`)
